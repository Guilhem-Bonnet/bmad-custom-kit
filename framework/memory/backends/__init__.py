...
def get_backend(config_override: dict | None = None) -> tuple:
    ...
    Ordre de priorité :
    1. ENV BMAD_OLLAMA_URL → ollama
    2. ENV BMAD_QDRANT_URL → qdrant-server
    ...
    env_ollama = os.environ.get("BMAD_OLLAMA_URL", "")
    env_qdrant = os.environ.get("BMAD_QDRANT_URL", "")
...
def _auto_detect(mem_cfg: dict) -> str:
    ...
    qdrant_url = mem_cfg.get("qdrant_url", os.environ.get("BMAD_QDRANT_URL", ""))
    ollama_url = mem_cfg.get("ollama_url", os.environ.get("BMAD_OLLAMA_URL", "http://localhost:11434"))
...
def _instantiate(backend_name: str, mem_cfg: dict, env_ollama: str, env_qdrant: str) -> tuple:
    ...
    collection = mem_cfg.get("collection_prefix", "bmad")
...
def _warn_connection(backend: str, url: str, err: Exception) -> None:
    ...
    print(f"   → Vérifier BMAD_OLLAMA_URL / BMAD_QDRANT_URL ou lancer le service")
    print(f"   → Fallback backend local JSON (fonctionnel, recherche par mots-clés)")
